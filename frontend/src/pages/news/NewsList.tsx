...
export default function NewsList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => newsApi.list({ page, limit: 10, search, status }).then((r) => r.data),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => newsApi.delete(id),
    onSuccess: () => { toast.success('News item deleted'); qc.invalidateQueries({ queryKey: ['news'] }); setDeleteTarget(null); },
    onError: () => toast.error('Failed to delete'),
  });

  const toggleMutation = useMutation({
    mutationFn: (id: string) => newsApi.toggleStatus(id),
    onSuccess: () => { toast.success('Status updated'); qc.invalidateQueries({ queryKey: ['news'] }); },
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-center justify-between">
        ...
        <Link to="/news/new"><Button><PlusIcon className="w-4 h-4" /> New News Item</Button></Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        {isLoading ? <div className="p-8 text-center"><div className="animate-spin w-6 h-6 border-2 border-brand-500 border-t-transparent rounded-full mx-auto" /></div>
          : !data?.data.length ? (
            <div className="p-12 text-center text-gray-400"><p>No news items found</p></div>
          ) : (
            <>
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="px-4 py-3 text-left font-medium text-gray-500">Publish Date</th>
                  ...
                  {data.data.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-900 truncate max-w-xs">{item.headline_en}</p>
                        {item.headline_ar && <p className="text-xs text-gray-400 truncate" dir="rtl">{item.headline_ar}</p>}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => toggleMutation.mutate(item.id)}><StatusBadge status={item.status} /></button>
                      </td>
      ...
      <ConfirmModal open={!!deleteTarget} onClose={() => setDeleteTarget(null)}
        ...
        loading={deleteMutation.isPending} title="Delete News Item"
        message={`Delete "${deleteTarget?.headline_en}"? This cannot be undone.`} />
    </div>
  );
}
