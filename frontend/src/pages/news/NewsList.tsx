...
export default function NewsList() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('✓'); qc.invalidateQueries({ queryKey: ['news'] }); setDeleteTarget(null); },
    onError: () => toast.error('!'),
  });
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        {isLoading ? <div className="p-8 text-center"><div className="animate-spin w-6 h-6 border-2 border-brand-500 border-t-transparent rounded-full mx-auto" /></div>
          : !data?.data.length ? <div className="p-12 text-center text-gray-400"><p>{T.news.no_results}</p></div>
          : (
            <>
      ...
      <ConfirmModal open={!!deleteTarget} onClose={() => setDeleteTarget(null)}
        ...
        loading={deleteMutation.isPending} title={T.news.delete_title}
        message={T.news.delete_msg(deleteTarget?.headline_en || '')} />
    </div>
  );
}
